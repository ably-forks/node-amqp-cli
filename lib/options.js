...
function Options(options, usageOptions) {
  ...
  var defaultOptions = [
    ...
    {name: 'user', alias: 'u', type: String, description: 'username', group: 'server' },
    {name: 'password', alias: 'P', type: String, description: 'password (leave blank to be prompted for the password)', group: 'server' },
    {name: 'creds', alias: 'C', type: String, description: 'username and password in format username:password', group: 'server', defaultValue: ''},
...
Options.prototype.getAmqpOptions = function () {
  var options = {
    ...
    user: this.options.server.user || (this.options.server.creds ? this.options.server.creds.split(':')[0] : undefined),
    password: this.options.server.password || (this.options.server.creds ? this.options.server.creds.split(':')[1] : undefined),
...
Options.prototype.pwdPrompt = function (next) {
  if (this.options.server.password || this.options.server.creds) {
    next();
    return;
  }
